...
public class SobotPhotoActivity extends BaseActivity implements View.OnLongClickListener{
	...
	protected int getLayoutId() {
		return ResourceUtils.getIdByName(this, "layout",
	...
	private void showGif(String savePath) {
		...
		try {
			...
			int w = SizeUtils.dp2px(bitmap.getWidth());
			int h = SizeUtils.dp2px(bitmap.getHeight());
			if (w == h) {
				if (w > screenWidth) {
					w = screenWidth;
					h = w;
