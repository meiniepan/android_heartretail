...
import com.sobot.chat.dengy.viewHolder.OrderMessageHolder;
import com.sobot.chat.dengy.viewHolder.ProductMessageHolder;
...
public class SobotMsgAdapter extends SobotBaseAdapter<ZhiChiMessageBase> {
    ...
    private static final String[] layoutRes = {
            ...
            "sobot_chat_msg_item_product",//商品咨询内容的布局文件
            "sobot_chat_msg_item_order",//订单咨询内容的布局文件
            "sobot_chat_msg_item_product",//商品咨询内容的布局文件
            "sobot_chat_msg_item_order",//订单咨询内容的布局文件
    };
     ...
     * 商品卡信息
     */
    public static final int MSG_TYPE_PRODUCT = 9;// TODO: 2017/10/19 自定义商品卡
    /**
     * 订单消息
     */
    public static final int MSG_TYPE_ORDER = 10;// TODO: 2017/10/19 自定义商品卡
    /**
     * 商品卡信息带发送
     */
    public static final int MSG_TYPE_PRODUCT_SEND = 11;// TODO: 2017/10/19 自定义商品卡带发送
    /**
     * 订单消息带发送
     */
    public static final int MSG_TYPE_ORDER_SEND = 12;// TODO: 2017/10/19 自定义商品卡带发送
    ...
    private View initView(View convertView, int itemType, int position, final ZhiChiMessageBase message) {
        if (convertView == null) {
            ...
            switch (itemType) {
                ...
                case MSG_TYPE_PRODUCT:
                    // TODO: 2017/10/23 新的消息类型 商品卡
                    holder = new ProductMessageHolder(context, convertView, false);
                    break;
                case MSG_TYPE_ORDER:
                    // TODO: 2017/10/23 新的消息类型 订单卡
                    holder = new OrderMessageHolder(context, convertView, false);
                    break;
                case MSG_TYPE_PRODUCT_SEND:
                    // TODO: 2017/10/23 新的消息类型 商品卡带发送
                    holder = new ProductMessageHolder(context, convertView, true);
                    break;
                case MSG_TYPE_ORDER_SEND:
                    // TODO: 2017/10/23 新的消息类型 订单卡带发送
                    holder = new OrderMessageHolder(context, convertView, true);
                    break;
        ...
        try {
            ...
            if (ZhiChiConstant.message_sender_type_customer == senderType
                ...
                if (message.getAnswer() != null) {
                    if (ZhiChiConstant.message_type_text == Integer
                        ...
                        if (message.getAnswer().getMsg() != null &&
                                message.getAnswer().getMsg().contains("[消息类型]")
                                && message.getAnswer().getMsg().contains("[订单]") || message.getAnswer().getMsg().contains("[售后]")) {
                            return MSG_TYPE_ORDER;// TODO: 2017/11/7 订单类型
                        } else if (message.getAnswer().getMsg() != null &&
                                message.getAnswer().getMsg().contains("[消息类型]")
                                && message.getAnswer().getMsg().contains("[商品]")) {
                            return MSG_TYPE_PRODUCT;// TODO: 2017/11/7 商品类型

                        } else if (ZhiChiConstant.message_sender_type_robot == Integer
                ...
                return MSG_TYPE_AUDIO_R;
                    ...
                    .parseInt(message.getSenderType())) {
                if (message.getT() != null && message.getT().contains("订单")) {
                    return MSG_TYPE_ORDER_SEND;
                } else if (message.getT() != null && message.getT().contains("商品")) {
                    return MSG_TYPE_PRODUCT_SEND;
                } else {
