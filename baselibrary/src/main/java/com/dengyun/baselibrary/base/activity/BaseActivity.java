...
//    protected  void initData(){};
    ...
    /**
     * 错误布局的点击监听（按钮的id必须是error_retry_view）
     */
    @Override
    public void setOnErrorRetryClickListener(View.OnClickListener onErrorRetryClickListener) {
        ...
        if (contentView instanceof StateRelativeLayout) {
            ((StateRelativeLayout) contentView).setOnErrorRetryClickListener(onErrorRetryClickListener);
        } else if (contentView instanceof StateLinearLayout) {
            ((StateLinearLayout) contentView).setOnErrorRetryClickListener(onErrorRetryClickListener);
        } else if (contentView instanceof StateFrameLayout) {
            ((StateFrameLayout) contentView).setOnErrorRetryClickListener(onErrorRetryClickListener);
        }
    }

    /**
     * 空布局的点击监听（按钮的id必须是empty_retry_view）
     */
    @Override
    public void setOnEmptyRetryClickListener(View.OnClickListener onEmptyRetryClickListener) {
        if (null == contentView) {
            contentView = getContentView();
        }
        if (contentView instanceof StateRelativeLayout) {
            ((StateRelativeLayout) contentView).setOnEmptyRetryClickListener(onEmptyRetryClickListener);
        } else if (contentView instanceof StateLinearLayout) {
            ((StateLinearLayout) contentView).setOnEmptyRetryClickListener(onEmptyRetryClickListener);
        } else if (contentView instanceof StateFrameLayout) {
            ((StateFrameLayout) contentView).setOnEmptyRetryClickListener(onEmptyRetryClickListener);
        }
    }

    /**
     * 没网络的布局的点击监听（按钮的id必须是no_network_retry_view）
     */
    @Override
    public void setOnNoNetRetryClickListener(View.OnClickListener onNoNetRetryClickListener) {
        if (null == contentView) {
            contentView = getContentView();
        }
        if (contentView instanceof StateRelativeLayout) {
            ((StateRelativeLayout) contentView).setOnNoNetRetryClickListener(onNoNetRetryClickListener);
        } else if (contentView instanceof StateLinearLayout) {
            ((StateLinearLayout) contentView).setOnNoNetRetryClickListener(onNoNetRetryClickListener);
        } else if (contentView instanceof StateFrameLayout) {
            ((StateFrameLayout) contentView).setOnNoNetRetryClickListener(onNoNetRetryClickListener);
        }
    }
}
